...
# ------------------------------------------------------------------------------
# 8.1 METHOD-BASED API (FLUENT WRAPPERS)
# ------------------------------------------------------------------------------
...
def _resolve_border(style: Union[str, Tuple[str, ...], None]) -> Optional[Tuple[str, ...]]:
    if style is None:
        return None
    if isinstance(style, tuple):
        return style
    if isinstance(style, str):
        return _BORDER_MAP.get(style.strip().lower(), None)
    return None


def _resolve_align(align: Union[str, Align, None]) -> Align:
    if align is None:
        return Align.LEFT
    if isinstance(align, Align):
        return align
    if isinstance(align, str):
        return _ALIGN_MAP.get(align.strip().lower(), Align.LEFT)
    return Align.LEFT


class Component:
    """Base class for fluent, method-based components."""
    def render(self) -> str:
...
class Panel(Component):
    def __init__(
        ...
        color: str = "",
        border: Union[str, Tuple[str, ...], None] = None,
        align: Union[str, Align] = Align.LEFT,
        padding: int = 1,
        min_width: int = 10,
        gradient: Optional[Tuple[Tuple[int, int, int], Tuple[int, int, int]]] = None,
        gradient_dir: str = "vertical"
    ):
    ...
    def shadow(self, color: str = "", char: str = "█", offset_x: int = 1, offset_y: int = 1):
        self._shadow = (color, char, offset_x, offset_y)
        return self

    def render(self) -> str:
        style = _resolve_border(self.border)
        align = _resolve_align(self.align)
        if self.gradient:
            rendered = box_gradient(
                self.content,
                title=self.title,
                width=self.width,
                gradient=self.gradient,
                direction=self.gradient_dir,
                color=self.color,
                style=style,
                align=align,
        ...
        if self._shadow:
            sc, ch, ox, oy = self._shadow
            return shadow(rendered, color=sc, char=ch, offset_x=ox, offset_y=oy)
        return rendered
...
class Columns(Component):
    def __init__(
        ...
        items: List[Any],
        spacing: int = 2,
        alignments: List[Union[str, Align]] = None,
        widths: List[Union[int, str]] = None
    ):
        self.items = items
    ...
    def render(self) -> str:
        ...
        for item in self.items:
            if hasattr(item, "render"):
                cols_items.append(item.render())
            else:
                cols_items.append(str(item))
        aligns = None
        if self.alignments:
            aligns = [_resolve_align(a) for a in self.alignments]
        return cols(cols_items, spacing=self.spacing, alignments=aligns, widths=self.widths)
...
class Table(Component):
    def __init__(
        ...
        footers: List[str] = None,
        color: str = "",
        align: List[Union[str, Align]] = None,
        max_col_width: Union[int, str] = 40,
        widths: List[Union[int, str]] = None,
        padding: int = 1,
        wrap: bool = True,
        style: Union[str, Tuple[str, ...], None] = None,
        row_sep: bool = False,
        expand: bool = False
    ):
    ...
    def render(self) -> str:
        aligns = None
        if self.align:
            aligns = [_resolve_align(a) for a in self.align]
        style = _resolve_border(self.style)
        return table(
            self.headers,
            self.rows,
...
class Divider(Component):
    def __init__(self, text: str = "", width: Union[int, str] = 0, color: str = "", char: str = None):
        self.text = text
        self.width = width
        self.color = color
        self.char = char
    ...
    def render(self) -> str:
        if self.text:
            return divider(self.text, width=self.width, color=self.color, char=self.char)
        return hline(width=self.width, color=self.color, char=self.char)
...
class KVList(Component):
    def __init__(self, items: List[Tuple[str, Any]], **kwargs):
        self.items = items
        self.kwargs = kwargs

    def render(self) -> str:
        return kvlist(self.items, **self.kwargs)


class Tree(Component):
    def __init__(self, data: Union[Dict, List, str], **kwargs):
        self.data = data
        self.kwargs = kwargs

    def render(self) -> str:
        return tree(self.data, **self.kwargs)
...
class Tabs(Component):
    ...
    def render(self) -> str:
        return tabs(self.options, **self.kwargs)
...
class Breadcrumbs(Component):
    ...
    def render(self) -> str:
        return breadcrumbs(self.path, **self.kwargs)
...
class Sparkline(Component):
    ...
    def render(self) -> str:
        return sparkline(self.data, **self.kwargs)
...
class Barchart(Component):
    ...
    def render(self) -> str:
        return barchart(self.data, **self.kwargs)


class Progress(Component):
    def __init__(self, current: int, total: int, **kwargs):
        self.current = current
        self.total = total
        self.kwargs = kwargs

    def render(self) -> str:
        return progress(self.current, self.total, **self.kwargs)
...
class Markdown(Component):
    ...
    def render(self) -> str:
        return markdown_to_ansi(self.text)


class SelectPrompt(Component):
    def __init__(self, options: List[str], **kwargs):
        self.options = options
        self.kwargs = kwargs

    def run(self):
        return select(self.options, **self.kwargs)

    def choose(self):
        res = self.run()
        if isinstance(res, list):
            return [self.options[i] for i in res]
        if res == -1:
            return None
...
class ConfirmPrompt(Component):
    def __init__(self, message: str, **kwargs):
        self.message = message
        self.kwargs = kwargs

    def run(self) -> bool:
        return confirm(self.message, **self.kwargs)

    def render(self) -> str:
        return ""


class InputPrompt(Component):
    def __init__(self, message: str, **kwargs):
        self.message = message
        self.kwargs = kwargs

    def run(self) -> str:
        return prompt(self.message, **self.kwargs)
...
class Notification(Component):
    ...
    def run(self):
        return notification(self.msg, **self.kwargs)
...
class Tooltip(Component):
    ...
    def render(self) -> str:
        return tooltip(self.text, **self.kwargs)


class UI:
    """
    Method-based entry point:
      ui.panel(...).shadow().render()
      ui.table(...).render()
      ui.columns([...]).render()
    """
    def panel(self, content: Any, **kwargs) -> Panel:
        return Panel(content, **kwargs)

    def columns(self, items: List[Any], **kwargs) -> Columns:
        return Columns(items, **kwargs)

    def table(self, headers: List[str], rows: List[List[Any]], **kwargs) -> Table:
        return Table(headers, rows, **kwargs)

    def divider(self, text: str = "", **kwargs) -> Divider:
        return Divider(text=text, **kwargs)

    def hr(self, **kwargs) -> Divider:
    ...
    def box(self, content: Any, **kwargs) -> Panel:
        return Panel(content, **kwargs)

    def box_gradient(self, content: Any, **kwargs) -> Panel:
        return Panel(content, **kwargs)

    def kvlist(self, items: List[Tuple[str, Any]], **kwargs) -> KVList:
        return KVList(items, **kwargs)

    def tree(self, data: Union[Dict, List, str], **kwargs) -> Tree:
        return Tree(data, **kwargs)

    def tabs(self, options: List[str], **kwargs) -> Tabs:
        return Tabs(options, **kwargs)

    def breadcrumbs(self, path: List[str], **kwargs) -> Breadcrumbs:
        return Breadcrumbs(path, **kwargs)

    def sparkline(self, data: List[float], **kwargs) -> Sparkline:
        return Sparkline(data, **kwargs)

    def barchart(self, data: Dict[str, float], **kwargs) -> Barchart:
        return Barchart(data, **kwargs)

    def progress(self, current: int, total: int, **kwargs) -> Progress:
        return Progress(current, total, **kwargs)
    ...
    def theme(self, **kwargs) -> Theme:
        return Theme(**kwargs)

    def skin(self, name: str, **kwargs) -> Skin:
        return Skin(name=name, **kwargs)

    def border(self, name: Union[str, Tuple[str, ...]] = "rounded"):
        return _resolve_border(name) or Border.ROUNDED

    def align(self, name: Union[str, Align] = "left") -> Align:
        return _resolve_align(name)

    def layout(self, name: Union[str, Layout] = "vertical") -> Layout:
        ...
        if isinstance(name, str):
            ...
            if n in ("horizontal", "h"):
                return Layout.HORIZONTAL
            if n in ("grid", "g"):
                return Layout.GRID
        return Layout.VERTICAL

    def overlay(self, base: str, overlay_content: str, x: int = 0, y: int = 0) -> str:
        return overlay(base, overlay_content, x=x, y=y)

    def shadow(self, content: str, **kwargs) -> str:
        return shadow(content, **kwargs)

    def shadow_box(self, content: Any, **kwargs) -> str:
        return shadow_box(content, **kwargs)

    def floating_box(self, content: Any, **kwargs) -> str:
        return floating_box(content, **kwargs)

    def calendar(self, **kwargs) -> str:
        return calendar_widget(**kwargs)

    def hline(self, **kwargs) -> Divider:
        return Divider(text="", **kwargs)

    def responsive_cols(self, columns: List[Union[List[str], str]], **kwargs) -> str:
        return responsive_cols(columns, **kwargs)

    def select(self, options: List[str], **kwargs) -> SelectPrompt:
        return SelectPrompt(options, **kwargs)

    def confirm(self, message: str, **kwargs) -> ConfirmPrompt:
        return ConfirmPrompt(message, **kwargs)

    def prompt(self, message: str, **kwargs) -> InputPrompt:
        return InputPrompt(message, **kwargs)

    def notification(self, msg: str, **kwargs) -> Notification:
        return Notification(msg, **kwargs)

    def tooltip(self, text: str, **kwargs) -> Tooltip:
        return Tooltip(text, **kwargs)

    def success(self, msg: str, **kwargs):
        return success(msg, **kwargs)

    def error(self, msg: str, **kwargs):
        return error(msg, **kwargs)

    def warn(self, msg: str, **kwargs):
        return warn(msg, **kwargs)

    def info(self, msg: str, **kwargs):
        return info(msg, **kwargs)

    def step(self, msg: str, **kwargs):
        return step(msg, **kwargs)

    def bullet(self, msg: str, **kwargs):
        return bullet(msg, **kwargs)

    def exception(self, e: Exception, **kwargs):
        return exception_box(e, **kwargs)

    def spinner(self, text: str = "Processing", **kwargs) -> "Spinner":
        return Spinner(text, **kwargs)

    def progress_bar(self, total: int = 0, **kwargs) -> ProgressBar:
        return ProgressBar(total=total, **kwargs)

    def live(self, **kwargs) -> Live:
        return Live(**kwargs)

    def fullscreen(self, **kwargs) -> Fullscreen:
        return Fullscreen(**kwargs)

    def grid(self, columns: int = 2, padding: int = 2, vertical_spacing: int = 0) -> Grid:
        return Grid(columns=columns, padding=padding, vertical_spacing=vertical_spacing)

    def dashboard(self, mode: Layout = Layout.VERTICAL, spacing: int = 2, vertical_spacing: int = 1) -> Dashboard:
        return Dashboard(mode=mode, spacing=spacing, vertical_spacing=vertical_spacing)

    def menu(self, options: List[str], **kwargs) -> Menu:
        return Menu(options, **kwargs)

    def logger(self, name: str = "APP", level: str = "INFO") -> Logger:
        return Logger(name=name, level=level)

    def cli(self, description: str = "") -> CLI:
        return CLI(description=description)

    def set_theme(self, name: str):
        return set_theme(name)

    def set_skin(self, name_or_skin: Union[str, Skin]):
        return set_skin(name_or_skin)

    def register_skin(self, name: str, skin: Skin):
        return register_skin(name, skin)

    def set_unicode(self, enabled: bool = True):
        return set_unicode(enabled)

    def fnum(self, n: Union[int, float], decimals: int = 0) -> str:
        return fnum(n, decimals=decimals)

    def ftime(self, seconds: float, compact: bool = True) -> str:
        return ftime(seconds, compact=compact)

    def fsize(self, size_bytes: int, precision: int = 1) -> str:
        return fsize(size_bytes, precision=precision)

    def fpercent(self, value: float, total: float = 100, decimals: int = 1, colored: bool = False) -> str:
        return fpercent(value, total=total, decimals=decimals, colored=colored)

    def ftrunc(self, text: str, max_len: int, suffix: str = "...") -> str:
        return ftrunc(text, max_len=max_len, suffix=suffix)

    def run_textual_demo(self):
        return run_textual_demo()

    def window(self, title: str = "Window", **kwargs):
        _require_textual()
        return DraggableWindow(title, **kwargs)

    def textual_app(self):
        _require_textual()
        return WindowDemo()


# Singleton for method-based API
ui = UI()
...
class Live:
    ...
    def _run(self):
        ...
        while not self._stop_event.is_set():
            ...
            remaining = last_lines - len(output)
            if remaining > 0 and Term.is_tty:
                for _ in range(remaining):
                    Term.clear_line()
                    print()
                Term.cursor_up(remaining)
